import electron, { app as app$1, nativeTheme, net, ipcMain as ipcMain$1, BrowserWindow, dialog, shell as shell$1, webContents, nativeImage, Tray, Menu } from "electron";
import fs, { existsSync, mkdirSync, accessSync, constants as constants$1, appendFileSync, statSync, renameSync, readdirSync, readFileSync, rmSync, createWriteStream, writeFileSync as writeFileSync$1, cpSync } from "node:fs";
import path, { join, delimiter, dirname, sep } from "node:path";
...
const en = {
  ...
  "git.asarSizeMismatch": "app.asar size check failed (expected {want} bytes, got {got}); please retry",
...
class PtyManager {
  ...
  async start(cwd, title2, run) {
    ...
    const session = new PtySession(id2, title2, cwd, shell2, args, env2);
    session.on("exit", () => sessions.delete(id2));
    sessions.set(id2, session);
    return { id: id2, title: title2, cwd: session.cwd };
  }
  get(id2) {
...
async function verifyNodeRuntime() {
  ...
  if (!info.ok) {
    const msg = info.version ? m("err.nodeVersion", { version: info.version }) : m("err.nodeMissing");

...
/**
 * Beta OTA channel: an orphan branch alongside `release` that carries pre-releases. Kept
 * separate from the stable branch on purpose — the publisher only writes one branch per push,
 * so flipping the setting changes what the client *follows*, never what it already installed.
 */
export const RELEASE_BRANCH_BETA = 'release-beta'

/**
 * Cached "the beta branch isn't on the remote" verdict, so a channel pointing at a branch nobody
 * has published yet doesn't re-probe (and re-log) on every silent update survey. Reset when the
 * user switches channels.
 */
let betaBranchMissing = false

/** The branch the current channel follows, after the beta→stable fallback is applied. */
export function effectiveReleaseBranch(): string {
  const wantsBeta = getSettings().containerChannel === 'beta'
  return wantsBeta && !betaBranchMissing ? RELEASE_BRANCH_BETA : RELEASE_BRANCH
}

/** Called when the channel setting changes, so a stale "beta is missing" verdict is re-probed. */
export function resetBranchProbe(): void {
...
async function fetchTip(name: string, onProgress?: ProgressCb): Promise<ReleaseTip> {
  ...
  try {
    ...
    // A channel pointing at a branch that was never published isn't an error the user can fix:
    // fall back to stable for this and all later checks, and say so in the timeline.
    if (branch !== RELEASE_BRANCH) {
      betaBranchMissing = true
      logEvent({
        level: 'warn',
        kind: 'ota.channelFallback',
        detail: `${branch}: ${(err as Error).message}`,
...
/** One `git fetch --depth 1 origin <branch>`, streaming its percentage to the progress cb. */
function fetchBranch(branch: string, name: string, onProgress?: ProgressCb): Promise<string> {
  return runGitAsync(
...
export function relaunchToApplyStaged(): boolean {
  ...
  const metaFile = join(updatesRoot(), 'update-meta.json')
  let meta: { pendingAsar?: string | null; version?: string } | null = null
  try {
    meta = JSON.parse(readFileSync(metaFile, 'utf-8'))
  } catch {
    meta = null
  ...
  logEvent({
    level: 'info',
    kind: 'ota.applying',
    meta: { from: app.getVersion(), to: meta?.version || 'unknown' }
  })
...
export function rollbackToPreviousAsar(): boolean {
  ...
  try {
    ...
    logEvent({
      level: 'warn',
      kind: 'ota.rollback',
      meta: { from: app.getVersion(), to: rb.fromVersion || 'unknown' }
    })
    return true
  } catch (err) {

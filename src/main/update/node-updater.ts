...
/** Cap on offered versions (newest-first). The dropdown is now filterable, so a larger
    historical window is fine to browse — this only bounds the (already engines-filtered) list. */
...
/**
 * The official dist index, filtered to versions offering a win-x64 zip. Newest first.
 * By default only versions every hosted runtime accepts (nodeVersionUsable) are returned;
 * `includeIncompatible` widens the list to every win-x64 release, tagging the out-of-range
 * ones with `usable:false` so the UI can surface them behind a warning instead of hiding them.
 */
export async function listNodeVersions(includeIncompatible = false): Promise<NodeVersionInfo[]> {

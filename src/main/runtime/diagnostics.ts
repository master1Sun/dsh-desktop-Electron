...
/** Both per-page env maps share the shape (pageId -> VAR -> value) and the same risk. */
const ENV_MAP_KEYS = ['pageEnvs', 'pageCustomEnvs'] as const
function maskSettings(): Record<string, unknown> {
  const s = getSettings() as unknown as Record<string, unknown>
  const out: Record<string, unknown> = { ...s }
  // pageEnvs / pageCustomEnvs: { <pageId>: { <VAR>: <value> } } — mask per variable name.
  for (const mapKey of ENV_MAP_KEYS) {
    const envs = s[mapKey] as Record<string, Record<string, string>> | undefined
    if (!envs) continue
...
export async function exportDiagnostics(registry: PageRegistry): Promise<string | null> {
  ...
  try {
    ...
    // The activity timeline is the "what happened in order" half of a field report; the raw
    // logs above are the "what did it print" half. JSONL so a maintainer can grep or re-parse it.
    writeFileSync(
    ...
    // Recorded after the copy: the row itself can't be inside the bundle it describes.

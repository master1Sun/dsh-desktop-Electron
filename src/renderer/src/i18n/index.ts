...
const zh: Dict = {
  ...
  mcpMgr: {
    ...
    callIsError: '⚠ 服务报告错误：',
    callFailed: '调用失败：{err}',
    bridgeHint: '注册表会自动同步给被托管的 Agent：codex 启动前写入其 config.toml；所有页面注入下方文件路径（DSH_MCP_CONFIG_JSON / DSH_MCP_CATALOG）供自行读取。',
    copyPath: '复制路径',
...
const en: Dict = {
  ...
  mcpMgr: {
    ...
    emptyHint: 'Click “Add server” and give it a launch command, e.g. npx -y @modelcontextprotocol/server-filesystem',
    autoStart: 'auto-connect',
    disabled: 'disabled',
    builtinTag: 'built-in',
    builtinTip: 'Shipped by the container — cannot be edited or removed; connect it to browse and test its tools',
    toolsN: '{n} tools',
    connect: 'Connect',
    disconnect: 'Disconnect',
    edit: 'Edit',
    removeTitle: 'Remove MCP server',
    removeConfirm: 'Remove “{name}”? If connected it will be disconnected and its child process terminated first.',
    msgSaved: 'Saved',
    ...
    envKeyHint: 'This server needs a secret to work — fill in {key}=<your key> above and turn on “Enabled”',
    envKeyGet: 'Get a key',
    ...
    bridgeHint: 'The registry syncs to hosted agents automatically: codex gets it written into its config.toml before launch; every page receives the file paths below via DSH_MCP_CONFIG_JSON / DSH_MCP_CATALOG and may read them itself.',
    copyPath: 'Copy path',
    pkgMissing: 'this built-in server\'s component is not downloaded yet (offline / fresh install) —',

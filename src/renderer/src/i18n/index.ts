...
const zh: Dict = {
  ...
  settings: {
    ...
    layoutIm: '侧边栏',
    layoutModeTip:
      '经典：顶部菜单 + 居中浮层面板；侧边栏：紧凑标题栏 + 左侧图标栏 + 停靠面板。仅切换布局，毛玻璃质感、主题与背景不变。',
    behavior: '行为',
  ...
  mcpMgr: {
    ...
    callFailed: '调用失败：{err}',
    bridgeHint:
      '注册表会自动同步给被托管的 Agent：codex 启动前写入其 config.toml；所有页面注入下方文件路径（DSH_MCP_CONFIG_JSON / DSH_MCP_CATALOG）供自行读取。',
    copyPath: '复制路径',
  ...
  wsMgr: {
    title: '共享上下文',
    tag: '所有 Agent 可见',
    hint: '容器托管的每个 Agent 启动时都会收到工作目录与本文件两个环境变量（DSH_WORKSPACE_DIR / DSH_WORKSPACE_FILE），据此读写同一份上下文。这里修改的内容会立即对所有 Agent 生效。',
    dirLabel: '工作区目录',
    copyPath: '复制文件路径',
    fTask: '当前任务 / 目标',
...
const en: Dict = {
  ...
  settings: {
    ...
    layoutIm: 'Sidebar',
    layoutModeTip:
      'Classic: top menu bar + centered floating panels. Sidebar: a compact title bar + a left icon rail + a docked panel. Layout only — frosted surfaces, theme and background are unchanged.',
    behavior: 'Behavior',
  ...
  palette: {
    ...
    actLayout: 'Toggle layout (classic / sidebar)',
  ...
  mcpMgr: {
    ...
    bridgeHint:
      'The registry syncs to hosted agents automatically: codex gets it written into its config.toml before launch; every page receives the file paths below via DSH_MCP_CONFIG_JSON / DSH_MCP_CATALOG and may read them itself.',
    copyPath: 'Copy path',
    pkgMissing:
  ...
  wsMgr: {
    ...
    hint: 'Every hosted agent the container starts receives two env vars pointing at the working directory and this file (DSH_WORKSPACE_DIR / DSH_WORKSPACE_FILE) and reads/writes this one context through them. Edits here take effect for every agent immediately.',
